import {
  ...
  isMissingUserSubscriptionsTable,
} from "@/features/subscription/server/subscription-service"
import {
...
export async function exportAccountData(
  ...
  const [trackedRunsResult, jobApplicationsResult, subscriptionResult] =
    await Promise.all([
      supabase
        .from("tracked_runs")
        .select("*")
      ...
      getUserSubscriptionRecord(auth.user, auth.accessToken).catch(
  ...
  if (
    trackedRunsResult.error &&
    !isMissingTableError(trackedRunsResult.error)
  ) {
    throw trackedRunsResult.error
  }
  if (
    jobApplicationsResult.error &&
    !isMissingTableError(jobApplicationsResult.error)
  ) {
