...
import { BILLING_PROVIDER, createPolarClient, getPolarProProductId } from "@/lib/polar"
...
export async function createPolarPortalUrl(params: {
  ...
  try {
    ...
    throw normalizeError(error, {
      code: "UPSTREAM_ERROR",
      status: 502,
      userMessage: "Polar billing portal failed. Verify your Polar customer sync and access token.",
      context: "polar-portal",
    })
  }
}

export async function syncSubscriptionFromPolarCustomerState(payload: PolarCustomerStateLike): Promise<PlanSnapshot | null> {
  ...
  const activeSubscription =
    payload.activeSubscriptions.find((subscription) => subscription.productId === proProductId) ?? null

  const admin = createSupabaseAdminClient()
  const nextPlan = activeSubscription ? SUBSCRIPTION_PLAN.PRO : SUBSCRIPTION_PLAN.FREE
  const nextStatus =
  ...
  const nextPeriodEnd = activeSubscription ? activeSubscription.currentPeriodEnd.toISOString() : null
