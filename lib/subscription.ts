...
export type PremiumFeature = (typeof PREMIUM_FEATURE)[keyof typeof PREMIUM_FEATURE]
...
const SUBSCRIPTION_PLAN_VALUES = new Set<SubscriptionPlan>(Object.values(SUBSCRIPTION_PLAN))
const SUBSCRIPTION_STATUS_VALUES = new Set<SubscriptionStatus>(Object.values(SUBSCRIPTION_STATUS))
...
export function isSubscriptionPlan(value: unknown): value is SubscriptionPlan {
  return typeof value === "string" && SUBSCRIPTION_PLAN_VALUES.has(value as SubscriptionPlan)
}

export function isSubscriptionStatus(value: unknown): value is SubscriptionStatus {
  return typeof value === "string" && SUBSCRIPTION_STATUS_VALUES.has(value as SubscriptionStatus)
}
...
export function isPlanSnapshot(value: unknown): value is PlanSnapshot {
  ...
  return (
    ...
    (candidate.currentPeriodEnd === null || isIsoDateTime(candidate.currentPeriodEnd))
  )
}

export function isUserSubscriptionRecord(value: unknown): value is UserSubscriptionRecord {
  if (!value || typeof value !== "object") return false
  ...
  return (
    ...
    isSubscriptionPlan(candidate.plan) &&
    isSubscriptionStatus(candidate.status) &&
    (candidate.billing_provider === null || typeof candidate.billing_provider === "string") &&
    (candidate.provider_customer_id === null || typeof candidate.provider_customer_id === "string") &&
    (candidate.provider_subscription_id === null || typeof candidate.provider_subscription_id === "string") &&
    (candidate.current_period_end === null || isIsoDateTime(candidate.current_period_end)) &&
    isIsoDateTime(candidate.created_at) &&
    isIsoDateTime(candidate.updated_at)
  )
}

export function isSubscriptionActive(plan: SubscriptionPlan, status: SubscriptionStatus) {
  return plan === SUBSCRIPTION_PLAN.PRO && (status === SUBSCRIPTION_STATUS.ACTIVE || status === SUBSCRIPTION_STATUS.TRIALING)
}
...
export function getGuestPlanSnapshot(): PlanSnapshot {
  return {
    ...
    entitlements: getPlanEntitlements(SUBSCRIPTION_PLAN.FREE, SUBSCRIPTION_STATUS.INACTIVE),
    source: "guest",
    currentPeriodEnd: null,
  }
}

export function createPlanSnapshot(record: Pick<UserSubscriptionRecord, "plan" | "status" | "current_period_end">): PlanSnapshot {
  return {
    plan: record.plan,
...
export function canAccessFeature(snapshot: PlanSnapshot, feature: PremiumFeature) {
  switch (feature) {
