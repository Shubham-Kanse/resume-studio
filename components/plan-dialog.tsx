...
import { Crown, Sparkles, Target, X, FileCode2, LayoutDashboard, BriefcaseBusiness } from "lucide-react"
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <section className="rounded-[28px] border border-white/8 bg-black/12 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.28em] text-white/45">Free</p>
                <h3 className="mt-2 text-2xl font-semibold text-foreground">$0</h3>
                <p className="mt-2 text-sm text-muted-foreground">Core resume tooling for manual editing and scoring.</p>
              </div>
              <div className="text-[11px] font-medium uppercase tracking-[0.2em] text-white/55">
                {currentPlan === SUBSCRIPTION_PLAN.FREE ? "Current plan" : "Available"}
              </div>
            </div>
            ...
            <div className="mt-6 space-y-3">
              {FREE_FEATURES.map((feature) => {
                ...
                return (
                  <div key={feature.label} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-black/12 px-4 py-3">
          ...
          <section className={cn(
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-semibold text-foreground">Premium workflow</h3>
                <p className="mt-2 text-sm text-muted-foreground">
                  AI-assisted resume creation, deeper ATS guidance, and application tracking.
                </p>
              </div>
              <div className={cn(
                "text-[11px] font-medium uppercase tracking-[0.2em]",
                currentPlan === SUBSCRIPTION_PLAN.PRO ? "text-sky-100" : "text-sky-200/75"
              )}>
                {currentPlan === SUBSCRIPTION_PLAN.PRO ? "Current plan" : "Upgrade"}
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <Button
                ...
                onClick={() => void (isAuthenticated ? onBillingAction() : onOpenAuth())}
                disabled={isBillingActionLoading}
              >
                {currentPlan === SUBSCRIPTION_PLAN.PRO
                  ? (isBillingActionLoading ? "Opening Portal..." : "Manage Billing")
                  : isAuthenticated
                    ? (isBillingActionLoading ? "Opening Checkout..." : "Upgrade to Pro")
                    : "Sign in to Continue"}
              </Button>
              <Button
                type="button"
                ...
                onClick={onClose}
              >
                Continue with {currentPlan === SUBSCRIPTION_PLAN.PRO ? "Pro" : "Free"}
              </Button>
            </div>
          </section>
        </div>
